...
float CalculateShadowVisibility(DirectionalLight light, vec3 position)
{
   ...
   //          visibility += texture(shadowTextures, shadowCoord +
   //          vec4(shadowCoordOffset, 0, 0));
   //      }
   //  }
...
//     float attenuation =  pow(max(0, 1 - (distance * distance) / (lightRange *
//     lightRange)), 2);
...
//     vec3 diffuse = diffuseAlbedo * max(dot(N, L), 0) * lightBrightness;
//     vec3 specular = specularAlbedo * pow(max(dot(N, H), 0.0), specularPower)
//     * lightBrightness;
...
//    float zNear = 0.01;    // TODO: Replace by the zNear of your perspective
//    projection float zFar  = 10000.0; // TODO: Replace by the zFar  of your
//    perspective projection float depth = texture2D(depthTexture, uv).x;

//    return (2.0 * zNear) / (zFar + zNear - depth * (zFar - zNear));
// }

// Calculates a position in view space from a depth and a position on the
// screen.
