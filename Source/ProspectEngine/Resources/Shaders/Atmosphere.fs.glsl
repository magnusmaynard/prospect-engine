...
const float MAX_HEIGHT =
    atmosphere.OutterRadius - atmosphere.InnerRadius; // Thickness of atmosphere.
...
const vec3 INVERSE_WAVELENGTH =
    vec3(1.0f / pow(0.650f, 4), 1.0f / pow(0.570f, 4), 1.0f / pow(0.475f, 4));
...
vec2 RaySphereIntersection(const vec3 o, // Ray origin.
...
{
   vec2 outterNearFar =
       RaySphereIntersection(position, ray, EARTH_POSITION, atmosphere.OutterRadius);
...
float MiePhase(float g1, float g2, float cos1, float cos2)
{
   ...
   float bot =
       (1.0 + g2 - 2.0 * g1 * cos1); // Intermediate step to cancel out 2 of the square roots.
...
{
   float sampleHeight =
       (length(EARTH_POSITION - samplePoint) - atmosphere.InnerRadius) / MAX_HEIGHT;
...
vec3 InScattering(vec3 ray, vec3 near, vec3 far)
{
   ...
   for (int i = 0; i < IN_SCATTER_SAMPLES; i++)
   {
      ...
      vec2 sunNearFar =
          RaySphereIntersection(samplePoint, sunRay, EARTH_POSITION, atmosphere.OutterRadius);
      ...
      vec3 outScattering = exp(
          (-(sunOutScattering + cameraOutScattering)) * (INVERSE_WAVELENGTH * K_RAYLEIGH + K_MIE));
